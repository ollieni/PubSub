...
for subscribers in subscribers_range:
    ...
    run_command(f"make load PUBLISHERS={subscribers} SUBSCRIBERS=1")
    ...
    if overall_time is not None and overall_time < 5:
...
plt.title('Pub/Sub performance (CMWQ impl)')
plt.xlabel('Publishers no.')
plt.ylabel('time (s)')

plt.savefig('kecho_concurrent_performance3.png')
...
plt.savefig('kecho_concurrent_performance_test3.png')
